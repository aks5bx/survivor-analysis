...
Simple approach:
- % of contestants with identity X
- % of winners/FTC/merge with identity X  
- Difference = over/under performance
"""
...
def main():
    ...
    # Calculate challenge wins per castaway-season
    challenges_us = challenges[challenges['version'] == 'US']
    challenge_wins = challenges_us.groupby(['castaway_id', 'version_season']).agg({
        'won': 'sum'
    }).reset_index()
    challenge_wins.columns = ['castaway_id', 'version_season', 'total_challenge_wins']
    
    df = df.merge(challenge_wins, on=['castaway_id', 'version_season'], how='left')
    ...
    for _, row in df.iterrows():
        players.append({
            ...
            'made_merge': bool(row['made_merge']),
            'found_advantage': bool(row['found_advantage']),
            'won_individual_challenge': bool(row['won_any_challenge'])
        })
